...
function Img(name, extension, price, ratings, reviews, sale, displayName, description) {
  ...
  this.filepath = `./img/${this.name}.${this.extension}`;
  this.votes = 0;
  this.views = 0;
  allImgs.push(this);
}
...
function createInstances() {
  ...
  new Img('tauntaun', 'jpg', '14.99', ratings.threeAndHalf, '59', 'true', 'Tauntaun Sleeping Bag', 'Now your kids can snuggle quietly in warm, Tauntaun entrails! Tauntaun mauris vitae ultricies leo integer malesuada nunc. Nunc faucibus a pellentesque sit amet. Nec ullamcorper sit amet risus. In hac habitasse platea dictumst vestibulum rhoncus est pellentesque. Ut eu sem integer vitae justo eget magna fermentum iaculis. Convallis posuere morbi leo urna molestie at. Sed lectus vestibulum mattis ullamcorper velit sed ullamcorper morbi.');
...
if(localStorage.storedInstances) {
  ...
  for(var i = 0; i < strings.length; i++) {
    new Img(strings[i].name, strings[i].extension, strings[i].price, strings[i].ratings, strings[i].reviews, strings[i].sale, strings[i].displayName, strings[i].description);
    allImgs[i].views = strings[i].views;
    allImgs[i].votes = strings[i].votes;
  }
  renderVotes();
} else {
  remainingVotes = 25;
